...
function ProductCard({ p, onQuickView }) {
  ...
  return (
    <div
      ...
      style={{
        border: "1px solid #DCCBAE", borderRadius: "8px", padding: "14px", cursor: "pointer",
        display: "flex", flexDirection: "column", gap: "8px", background: "#FFFFFF",
      }}
    >
      <div style={{
        height: "150px", borderRadius: "6px", overflow: "hidden",
        background: gallery[0] ? `#F4EDE2` : `linear-gradient(155deg, ${p.color}, ${p.color}CC)`,
        backgroundImage: gallery[0] ? `url(${gallery[0]})` : undefined,
      ...
      {avg !== null && (
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <Stars value={avg} />
          <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "11px", color: "#8A6A6E" }}>({reviews.length})</span>
        </div>
      )}
      <button
...
function QuickViewModal({ p, onClose, onBuy }) {
  const gallery = (p.images && p.images.length ? p.images : (p.image ? [p.image] : []));
  const [activeImage, setActiveImage] = useState(gallery[0] || null);
  const [copied, setCopied] = useState(false);
  const [showReviews, setShowReviews] = useState(false);
  const [showDetails, setShowDetails] = useState(false);
  const [selectedSizes, setSelectedSizes] = useState(p.sizes && p.sizes.length ? [p.sizes[0]] : []);
  ...
  return (
    <div
      onClick={onClose}
      style={{
        position: "fixed", inset: 0, background: "#1A0F1177",
    ...
    >
      <div
        ...
        style={{
          background: "#FFFFFF", borderRadius: "10px", width: "440px", maxWidth: "100%",
          maxHeight: "90vh", overflowY: "auto", padding: "18px",
          fontFamily: "'Inter', sans-serif",
        }}
      >
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <button onClick={onClose} style={{ background: "none", border: "none", fontSize: "18px", cursor: "pointer", color: "#8A6A6E" }}>✕</button>
        </div>

        <div style={{
          height: "220px", borderRadius: "6px", overflow: "hidden",
          ...
          <button onClick={share} style={{ background: "#F4EDE2CC", border: "none", borderRadius: "4px", cursor: "pointer", fontSize: "13px", padding: "4px 6px" }}>
            {copied ? "Copied!" : "🔗 Share"}
          </button>
        </div>

        {gallery.length > 1 && (
          <div style={{ display: "flex", gap: "5px", overflowX: "auto", marginTop: "8px" }}>
            {gallery.map((url, i) => (
              <button
                key={url + i}
                onClick={() => setActiveImage(url)}
                style={{
                  width: "40px", height: "40px", borderRadius: "4px", padding: 0, cursor: "pointer",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginTop: "14px" }}>
          <div>
            <div style={{ fontFamily: "'Fraunces', serif", fontSize: "19px", color: "#2A1A1C", fontWeight: 600 }}>{p.name}</div>
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "12px", color: "#8A6A6E", marginTop: "2px" }}>{p.cat}</div>
          </div>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "17px", color: "#6E2A2E", fontWeight: 600 }}>${p.price}</div>
        </div>
        ...
        {p.description && (
          <div style={{ marginTop: "12px" }}>
            ...
            {showDetails && (
              <div style={{ marginTop: "6px", fontSize: "13px", color: "#5A4A46", whiteSpace: "pre-wrap", lineHeight: 1.5 }}>
                {p.description}
              </div>
            )}
          </div>
        ...
        {p.sizes && p.sizes.length > 0 && (
          <div style={{ marginTop: "14px" }}>
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "10px", color: "#8A6A6E", marginBottom: "6px", letterSpacing: "0.04em" }}>
              SIZE {selectedSizes.length > 1 ? `(${selectedSizes.length} selected)` : ""}
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
              {p.sizes.map((s) => (
                <button
                  ...
                  style={{
                    padding: "5px 11px", fontSize: "12px", borderRadius: "4px", cursor: "pointer",
                    border: selectedSizes.includes(s) ? "2px solid #6E2A2E" : "1px solid #DCCBAE",
                    background: selectedSizes.includes(s) ? "#F4EDE2" : "#FFFFFF",
                    color: "#2A1A1C", fontWeight: selectedSizes.includes(s) ? 700 : 400,
                  }}
                >
                  {s}
                </button>
            ...
            <div style={{ fontSize: "11px", color: "#8A6A6E", marginTop: "6px" }}>
              Tap to select — you can pick more than one size in the same order.
            </div>
          </div>
        )}

        <div style={{ marginTop: "14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "10px", color: "#8A6A6E", letterSpacing: "0.04em" }}>QUANTITY</div>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <button
              onClick={() => setQuantity((q) => Math.max(1, q - 1))}
              style={{ width: "28px", height: "28px", borderRadius: "4px", border: "1px solid #DCCBAE", background: "#FFFFFF", cursor: "pointer", fontSize: "16px", lineHeight: 1 }}
            >
              −
            </button>
            <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "14px", minWidth: "16px", textAlign: "center" }}>{quantity}</span>
            <button
        ...
        <div style={{ marginTop: "10px", display: "flex", justifyContent: "space-between", alignItems: "baseline", borderTop: "1px dashed #DCCBAE", paddingTop: "10px" }}>
          <span style={{ fontSize: "13px", color: "#8A6A6E" }}>Total</span>
          <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "18px", color: "#6E2A2E", fontWeight: 700 }}>${total.toFixed(2)}</span>
        </div>
        ...
        <div style={{ marginTop: "14px" }}>
          ...
          {showReviews && reviews.length > 0 && (
            <div style={{ borderTop: "1px dashed #DCCBAE", paddingTop: "8px", marginTop: "8px", display: "flex", flexDirection: "column", gap: "6px" }}>
              {reviews.map((r) => (
        ...
        {p.preorder && (
          <div style={{ fontSize: "12px", color: "#8A6A6E", fontStyle: "italic", marginTop: "10px" }}>
            Pre-order now — ships as soon as it's back in stock.
          </div>
        )}

        <button
          onClick={() => onBuy(p, { sizes: selectedSizes, quantity, total })}
          style={{
            marginTop: "16px", width: "100%", border: "none", background: "#4FB8AE", color: "#0F2E2A",
            fontFamily: "'IBM Plex Mono', monospace", fontSize: "13px", letterSpacing: "0.06em",
            textTransform: "uppercase", padding: "13px 0", borderRadius: "6px", cursor: "pointer", fontWeight: 700,
          }}
        >
          Buy now
        </button>
...
export default function Home() {
  ...
  const [checkoutSize, setCheckoutSize] = useState(null);
    ...
    <div style={{ minHeight: "100vh" }}>
      ...
      <section style={{ padding: "36px 32px 60px", background: "#F4EDE2" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: "16px" }}>
          ...
          {filtered.map((p) => (
            <ProductCard key={p.id} p={p} onQuickView={setQuickViewProduct} />
          ))}
        </div>
      </section>
      ...
      {quickViewProduct && (
        <QuickViewModal
          ...
          onBuy={(product, details) => {
            setQuickViewProduct(null);
            setCheckoutSize(details);
